...
Function RunFile(file As String) As Long
    On Error GoTo runfileerror
    Dim s2 As String
    Dim q
    q = Chr(34)
    s2 = Space(MAX_FILENAME_LEN)
    FindExecutable file, vbNullString, s2
    s2 = Split(s2, Chr(0))(0)
    RunFile = Shell(q & s2 & q & " " & q & file & q, vbNormalFocus)
    Exit Function
runfileerror:
    MsgBox "Error Running File: " & file & vbCrLf & "Description: " & Err.Number & " : " & Err.Description
    Exit Function
End Function
...
Function SysDir()
    ...
    Dim num
    retval = Space(255)
    num = GetSystemDirectory(retval, 255)
    SysDir = Left(retval, num)
End Function

Sub PrintHTMLFile(filename As String)
    On Error Resume Next
    Dim taskid As Double
    Dim q
    q = Chr(34)
    Dim path, mshtml
    mshtml = SysDir & "\mshtml.dll"
    If Dir(mshtml) = "" Then
        taskid = RunFile(filename)
    Else
        path = "rundll32.exe " & mshtml & ",PrintHTML %1 " & q & filename & q
